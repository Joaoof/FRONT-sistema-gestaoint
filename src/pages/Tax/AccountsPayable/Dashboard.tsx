...
function SummaryCard({ label, value, color, progress }: SummaryCardProps) {
    const tones = {
        red: { accent: 'bg-rose-500', iconBg: 'bg-rose-50 text-rose-700 ring-rose-100 dark:bg-rose-500/10 dark:text-rose-400 dark:ring-rose-500/20', bar: 'bg-rose-500' },
        yellow: { accent: 'bg-amber-500', iconBg: 'bg-amber-50 text-amber-700 ring-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:ring-amber-500/20', bar: 'bg-amber-500' },
        green: { accent: 'bg-emerald-500', iconBg: 'bg-emerald-50 text-emerald-700 ring-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:ring-emerald-500/20', bar: 'bg-emerald-500' },
    } as const;
    const t = tones[color];
        ...
        >
            ...
            <div className="flex items-center gap-2">
                <span className={`w-7 h-7 rounded-md ring-1 flex items-center justify-center ${t.iconBg}`}>
                    {color === 'red' ? <TrendingDown className="w-3.5 h-3.5" strokeWidth={2} /> : <FileText className="w-3.5 h-3.5" strokeWidth={2} />}
                </span>
        ...
        <>
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-white/[0.06]">
                    <thead>
                        <tr>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Fornecedor</th>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Descrição</th>
                            ...
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Valor</th>
                    ...
                    <tbody className="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-white/[0.06]">
                        {records.map((item) => {
                            ...
                            return (
                                <tr key={item.id} className={`hover:bg-gray-50 dark:hover:bg-slate-800 ${isOverdue ? 'bg-red-50/60 dark:bg-red-950/40' : ''}`}>
                                    <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">
                                        {item.supplierName}
                                        {item.supplier?.email && (
                                            <div className="text-[11px] text-slate-500 dark:text-slate-400">{item.supplier.email}</div>
                                        )}
                                    </td>
                                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-200">{item.description}</td>
                                    ...
                                    <td className="px-4 py-3 text-sm tabular-nums">
                                        {item.interestAccrued > 0 ? (
                                            <span className="text-red-600 dark:text-red-400" title={`${item.daysOverdue} dia(s) de atraso`}>+ {formatBRL(item.interestAccrued)}</span>
                                        ) : (
                                            <span className="text-slate-400">—</span>
                                        )}
                                    </td>
...
export function AccountsPayableDashboard() {
    ...
    const { data, loading } = useQuery<{ accountsPayableSummary: AccountSummary }>(
        GET_ACCOUNTS_PAYABLE_SUMMARY,
        { fetchPolicy: 'cache-and-network' },
    );

    const summary = data?.accountsPayableSummary ?? { total: 0, pending: 0, paid: 0, overdue: 0, countTotal: 0 };
        ...
        <div className="space-y-6 w-full">
            <div className="flex items-start justify-between gap-4 pb-5 border-b border-slate-200 dark:border-white/[0.06]">
                <div className="min-w-0">
                    ...
                    <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">Gestão de fornecedores, despesas e pagamentos com cálculo de juros</p>
                </div>
            ...
            >
                <SummaryCard label="Total a pagar" value={summary.total} color="red" />
                <SummaryCard label="Pendentes" value={summary.pending} color="yellow" progress={100 - paidPercentage} />
                <SummaryCard label="Pagos" value={summary.paid} color="green" progress={paidPercentage} />
                <SummaryCard label="Vencidos" value={summary.overdue} color="red" />
            </motion.div>
